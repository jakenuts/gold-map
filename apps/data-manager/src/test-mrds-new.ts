...
import { BoundingBox } from './services/wfs-base-client.js';
import type { MRDSFeature } from './services/usgs-mrds-client-new.js';
...
async function testMRDSClient() {
    try {
        ...
        if (features.length > 0) {
            ...
            // Show statistics
            const uniqueTypes = new Set(features.map((f: MRDSFeature) => f.properties.dep_type).filter(Boolean));
            const uniqueCommodities = new Set(features.map((f: MRDSFeature) => f.properties.commod1).filter(Boolean));
            const uniqueStatuses = new Set(features.map((f: MRDSFeature) => f.properties.development_status).filter(Boolean));
